 ...
 * - real synchronous AM demodulation with phase determination by atan2 implemented
...
float32_t LPF_spectrum = 0.08;
float32_t spectrum_display_scale = 10.0;
...
float32_t LP_Fpass = 2500;
uint32_t LP_F_help = 2500;
float32_t LP_Fstop = 2600;
...
float32_t float_buffer_L_original [BUFFER_SIZE * N_B];
float32_t float_buffer_R_original [BUFFER_SIZE * N_B];
...
#define       DEMOD_AM_ME2        9
#define       DEMOD_AM_ME3       10
#define       DEMOD_SAM          11 // synchronous AM demodulation
#define       DEMOD_SAM_USB      12 // synchronous AM demodulation
#define       DEMOD_SAM_LSB      13 // synchronous AM demodulation
#define       DEMOD_SAM_STEREO   14 // SAM, with pseude-stereo effect
#define       DEMOD_DCF77        15 // set the clock with the time signal station DCF77
#define       DEMOD_AUTOTUNE     16 // AM demodulation with autotune function
...
//  7750000 ,"VLF", DEMOD_AM, 3600,3600,0,  
  22500000,"LW", DEMOD_AM1, 3600,3600,0,
  63900000,"MW",  DEMOD_AM1, 3600,3600,0,
  248500000,"120M",  DEMOD_AM1, 3600,3600,0,
  350000000,"90M",  DEMOD_LSB, 3600,3600,6,
  390500000,"75M",  DEMOD_AM1, 3600,3600,4,
  502500000,"60M",  DEMOD_AM1, 3600,3600,7,
  598100000,"49M",  DEMOD_AM1, 3600,3600,0,
  712000000,"41M",  DEMOD_AM1, 3600,3600,0,
  942000000,"31M",  DEMOD_AM1, 3600,3600,0,
  1173500000,"25M", DEMOD_AM1, 3600,3600,0,
  1357000000,"22M", DEMOD_AM1, 3600,3600,0,
  1514000000,"19M",  DEMOD_AM1, 3600,3600,0,
  1748000000,"16M", DEMOD_AM1, 3600,3600,0,
  1890000000,"15M", DEMOD_AM1, 3600,3600,0,
  2145000000,"13M", DEMOD_AM1, 3600,3600,0,
  2567000000,"11M", DEMOD_AM1, 3600,3600,0
};
...
const DEMOD_Descriptor DEMOD [21] =
{
    //   DEMOD_n, name
    {  DEMOD_USB, " USB  "}, 
    {  DEMOD_LSB, " LSB  "}, 
...
uint8_t       demod_mode =        DEMOD_SAM;

// definitions for SAM mode & quadrature oscillator
float32_t m_NcoPhase = 0.0;
float32_t m_NcoFreq = 0.0;
float32_t m_NcoLLimit = - 100000.0; // 10
float32_t m_NcoHLimit =  100000.0;
float32_t z0 = 0.0;
float32_t m_z1 = 0.0;
float32_t yy0 = 0.0;
float32_t m_y1 = 0.0;
float32_t DC_ALPHA = 0.999;
// mit omega = 0.1 und tau 0.707 höre ich gute audio
// 0.01 / 0.707 hohes Fiepen
// 0.2 / 0.707 ganz gut
// 0.2 / 0.00707 sehr schnelles Fangen, dosenmäßige audio
// 0.2 / 0.000707 zu schnell, fiepsiges Wuseln
// 2.0 / 0.1 nur Rauschen
// 0.2 // 0.01 --> bis jetzt am besten
// 0.2 / 0.5 --> Super bei 16ksps
// 0.2 / 0.707 ---> super duper
//###############################################
// zur Zeit gut etabliert:
// NcoLLimit = - 100000.0; HLimit = 100000.0;
// omega = 0.02 / besser 0.05; tau = 0.707;

float32_t omega = 0.05; // how do I code an PLL bandwidth of 100 Hz ?
float32_t tau = 0.707; // PLL damping factor
float32_t m_PllAlpha = 2.0 * tau * omega;
float32_t m_PllBeta = omega * omega;
float32_t Sin = 0.0;
float32_t Cos = 0.0;
float32_t tmp_re = 0.0;
float32_t tmp_im = 0.0;
float32_t phzerror = 0.0;

// new synchronous AM PLL & PHASE detector
// wdsp Warren Pratt, 2016
float32_t pll_fmin = -4000.0;
...
// medium 0.6, 200
// fast 1.2, 500
float32_t zeta = 0.65; // PLL step response: smaller, slower response 1.0 - 0.1
float32_t omegaN = 200.0; // PLL bandwidth 50.0 - 1000.0
  
  //pll
float32_t omega_min = TPI * pll_fmin * DF / SR[SAMPLE_RATE].rate;
...
float32_t mtauR = exp(- DF / (SR[SAMPLE_RATE].rate * tauR)); //0.99948;
float32_t onem_mtauR = 1.0 - mtauR;
float32_t mtauI = exp(- DF / (SR[SAMPLE_RATE].rate * tauI)); //0.99999255955;
...
float32_t FIR_dec1_I_state [25 + BUFFER_SIZE * N_B - 1];
arm_fir_decimate_instance_f32 FIR_dec1_Q;
float32_t FIR_dec1_Q_state [25 + BUFFER_SIZE * N_B - 1];
float32_t FIR_dec1_coeffs[25];

arm_fir_decimate_instance_f32 FIR_dec2_I;
float32_t FIR_dec2_I_state [44 + BUFFER_SIZE * N_B / 4 - 1];
arm_fir_decimate_instance_f32 FIR_dec2_Q;
float32_t FIR_dec2_Q_state [44 + BUFFER_SIZE * N_B / 4 - 1];
float32_t FIR_dec2_coeffs[44];
...
float32_t FIR_int1_I_state [(8 / 2) + BUFFER_SIZE * N_B / 8 - 1];
arm_fir_interpolate_instance_f32 FIR_int1_Q;
float32_t FIR_int1_Q_state [(8 / 2) + BUFFER_SIZE * N_B / 8 - 1];
float32_t FIR_int1_coeffs[8];

arm_fir_interpolate_instance_f32 FIR_int2_I;
float32_t FIR_int2_I_state [(4 / 4) + BUFFER_SIZE * N_B / 4 - 1];
arm_fir_interpolate_instance_f32 FIR_int2_Q;
float32_t FIR_int2_Q_state [(4 / 4) + BUFFER_SIZE * N_B / 4 - 1];
float32_t FIR_int2_coeffs[4];
...
void setup() {
  ...
  // for the large queue sizes we need a lot of buffers
  AudioMemory(120);
  delay(100);
  ...
  sgtl5000_1.eqBands (0.7, 0.05); // (float bass, float treble) in % -100 to +100
 ...
 ****************************************************************************************/
    ...
    if(arm_fir_decimate_init_f32(&FIR_dec1_I, 25, 4, FIR_dec1_coeffs, FIR_dec1_I_state, BUFFER_SIZE * N_BLOCKS)) {
      Serial.println("Init of decimation failed");
      while(1);
    }
    if(arm_fir_decimate_init_f32(&FIR_dec1_Q, 25,  4, FIR_dec1_coeffs, FIR_dec1_Q_state, BUFFER_SIZE * N_BLOCKS)) {
      Serial.println("Init of decimation failed");
      while(1);
    }
    
    // Decimation filter 2, M2 = 2
    calc_FIR_coeffs (FIR_dec2_coeffs, 44, (float32_t)5100.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4.0);
    if(arm_fir_decimate_init_f32(&FIR_dec2_I, 44, 2, FIR_dec2_coeffs, FIR_dec2_I_state, BUFFER_SIZE * N_BLOCKS / 4)) {
      Serial.println("Init of decimation failed");
      while(1);
    }
    if(arm_fir_decimate_init_f32(&FIR_dec2_Q, 44, 2, FIR_dec2_coeffs, FIR_dec2_Q_state, BUFFER_SIZE * N_BLOCKS / 4)) {
      Serial.println("Init of decimation failed");
      while(1);
    }
...
//    calc_FIR_coeffs (FIR_int1_coeffs, 8, (float32_t)5000.0, 80, 0, 0.0, 12000);
    calc_FIR_coeffs (FIR_int1_coeffs, 8, (float32_t)5100.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4.0);
    if(arm_fir_interpolate_init_f32(&FIR_int1_I, 2, 8, FIR_int1_coeffs, FIR_int1_I_state, BUFFER_SIZE * N_BLOCKS / 8)) {
      Serial.println("Init of interpolation failed");
      while(1);  
    }
    if(arm_fir_interpolate_init_f32(&FIR_int1_Q, 2, 8, FIR_int1_coeffs, FIR_int1_Q_state, BUFFER_SIZE * N_BLOCKS / 8)) {
      Serial.println("Init of interpolation failed");
      while(1);  
    }    
...
//    calc_FIR_coeffs (FIR_int2_coeffs, 4, (float32_t)5000.0, 80, 0, 0.0, 24000);
    calc_FIR_coeffs (FIR_int2_coeffs, 4, (float32_t)5100.0, 80, 0, 0.0, (float32_t)SR[SAMPLE_RATE].rate);

    if(arm_fir_interpolate_init_f32(&FIR_int2_I, 4, 4, FIR_int2_coeffs, FIR_int2_I_state, BUFFER_SIZE * N_BLOCKS / 4)) {
      Serial.println("Init of interpolation failed");
      while(1);  
    }
    if(arm_fir_interpolate_init_f32(&FIR_int2_Q, 4, 4, FIR_int2_coeffs, FIR_int2_Q_state, BUFFER_SIZE * N_BLOCKS / 4)) {
      Serial.println("Init of interpolation failed");
      while(1);  
    }    
...
// read in 32 blocks á 128 samples in I and Q
    ...
    sp_R = Q_in_R.readBuffer();
     ...
     arm_q15_to_float (sp_L, &float_buffer_L_original[BUFFER_SIZE * i], BUFFER_SIZE); // convert int_buffer to float 32bit
     arm_q15_to_float (sp_R, &float_buffer_R_original[BUFFER_SIZE * i], BUFFER_SIZE); // convert int_buffer to float 32bit
     Q_in_L.freeBuffer();
     Q_in_R.freeBuffer();
//     blocks_read ++;
    }
...
//    if (ms_500.check() == 1)
    ...
    arm_mean_f32(float_buffer_L_original, BUFFER_SIZE * N_BLOCKS, &sample_mean);
    arm_max_f32(float_buffer_L_original, BUFFER_SIZE * N_BLOCKS, &sample_max, &max_index);
    arm_min_f32(float_buffer_L_original, BUFFER_SIZE * N_BLOCKS, &sample_min, &min_index);
 ...
 ***********************************************************************************************/
    ...
    arm_scale_f32 (float_buffer_L_original, IQ_amplitude_correction_factor, float_buffer_L_original, BUFFER_SIZE * N_BLOCKS);
    // IQ phase correction
    IQ_phase_correction(float_buffer_L_original, float_buffer_R_original, IQ_phase_correction_factor, BUFFER_SIZE * N_BLOCKS);
 ...
 **********************************************************************************/
    ...
    {   // float_buffer_L contains I = real values
        ...
        hh1 = - float_buffer_R_original[i + 1];
        hh2 =   float_buffer_L_original[i + 1];
            float_buffer_L_original[i + 1] = hh1;
            float_buffer_R_original[i + 1] = hh2;
        // xnew(2) = -xreal(2) - jximag(2)
        hh1 = - float_buffer_L_original[i + 2];
        hh2 = - float_buffer_R_original[i + 2];
            float_buffer_L_original[i + 2] = hh1;
            float_buffer_R_original[i + 2] = hh2;
        // xnew(3) = + ximag(3) - jxreal(3)
        hh1 =   float_buffer_R_original[i + 3];
        hh2 = - float_buffer_L_original[i + 3];
            float_buffer_L_original[i + 3] = hh1;
            float_buffer_R_original[i + 3] = hh2;
    }
 ...
 **********************************************************************************/
      ...
      // decimation-by-4
      arm_fir_decimate_f32(&FIR_dec1_I, float_buffer_L_original, float_buffer_L, BUFFER_SIZE * N_BLOCKS);
      arm_fir_decimate_f32(&FIR_dec1_Q, float_buffer_R_original, float_buffer_R, BUFFER_SIZE * N_BLOCKS);
      // lowpass filter 5kHz, 80dB stopband att, fstop = 7k, 44 taps
      // decimation-by-2
      arm_fir_decimate_f32(&FIR_dec2_I, float_buffer_L, float_buffer_L_2, BUFFER_SIZE * N_BLOCKS / 4);
      arm_fir_decimate_f32(&FIR_dec2_Q, float_buffer_R, float_buffer_R_2, BUFFER_SIZE * N_BLOCKS / 4);
...
//    if(0)
    ...
    float32_t sample_mean = 0.0;
    uint32_t min_index, max_index;
    arm_mean_f32(float_buffer_L_2, BUFFER_SIZE * N_BLOCKS / 8, &sample_mean);
    arm_max_f32(float_buffer_L_2, BUFFER_SIZE * N_BLOCKS / 8, &sample_max, &max_index);
    arm_min_f32(float_buffer_L_2, BUFFER_SIZE * N_BLOCKS/ 8, &sample_min, &min_index);
...
// 6 a.) fill FFT_buffer with last events audio samples
    ...
    // copy recent samples to last_sample_buffer for next time!
      for(i = 0; i < BUFFER_SIZE * N_BLOCKS / 8; i++)
      {
         last_sample_buffer_L [i] = float_buffer_L_2[i]; 
         last_sample_buffer_R [i] = float_buffer_R_2[i]; 
      }
...
// 6. b) now fill audio samples into FFT_buffer (left channel: re, right channel: im)
      for(i = 0; i < BUFFER_SIZE * N_BLOCKS / 8; i++)
      {
        FFT_buffer[FFT_length + i * 2] = float_buffer_L_2[i]; // real
        FFT_buffer[FFT_length + i * 2 + 1] = float_buffer_R_2[i]; // imaginary
      }
...
// which one and how they are combined is dependent upon the demod_mode . . .
      ...
      if(demod_mode == DEMOD_SAM || demod_mode == DEMOD_SAM_LSB ||demod_mode == DEMOD_SAM_USB ||demod_mode == DEMOD_SAM_STEREO)
      {   // Wheatley 2011 cuteSDR & Warren Pratt´s WDSP, 2016
        for(i = 0; i < FFT_length / 2; i++)
        {
            Sin = sinf(phs);
            Cos = cosf(phs);
            tmp_re = Cos * iFFT_buffer[FFT_length + i * 2] - Sin * iFFT_buffer[FFT_length + i * 2 + 1];
            tmp_im = Cos * iFFT_buffer[FFT_length + i * 2 + 1] + Sin * iFFT_buffer[FFT_length + i * 2];
//            phzerror = -atan2(tmp_im, tmp_re);
            phzerror = atan2(tmp_im, tmp_re);
            if(WDSP_SAM)
            { // 72% @ 32ksps
              // now with decimation: 52% @ 96ksps
                del_out = fil_out;
            ...
            {
            m_NcoFreq += (m_PllBeta * phzerror);
            if(m_NcoFreq > m_NcoHLimit) m_NcoFreq = m_NcoHLimit;
            else if(m_NcoFreq < m_NcoLLimit) m_NcoFreq = m_NcoLLimit;
            phs += (m_NcoFreq + m_PllAlpha * phzerror);
            }
         if(demod_mode != DEMOD_SAM)
         {
         // TODO
         // Hilbert I
         // Bandpass 0 degrees
         ...
         // add or substract according to desired sideband
         // SAM_USB / SAM_LSB
         // SAM_STEREO
            ...
            float_buffer_L[i] = tmp_re;
            }
            else
            {   
            z0 = tmp_re + (m_z1 * DC_ALPHA);
//            yy0 = tmp_im + (m_y1 * DC_ALPHA);
            float_buffer_L[i] = (z0 - m_z1);
//            float_buffer_R[i] = (yy0 - m_y1);
//            m_y1 = yy0;
            m_z1 = z0;
            }
            // wrap round 2PI, modulus
            while (phs >= TPI) phs -= TPI;
            while (phs < 0.0) phs += TPI;
        } 
        if(demod_mode != DEMOD_SAM_STEREO) arm_copy_f32(float_buffer_L, float_buffer_R, FFT_length/2);
      }
      else
...
//      arm_fir_interpolate_f32(&FIR_int1_Q, float_buffer_R, &iFFT_buffer[1024], BUFFER_SIZE * N_BLOCKS / 8);
      arm_fir_interpolate_f32(&FIR_int1_I, float_buffer_L, float_buffer_L_2, BUFFER_SIZE * N_BLOCKS / 8);
      arm_fir_interpolate_f32(&FIR_int1_Q, float_buffer_R, float_buffer_R_2, BUFFER_SIZE * N_BLOCKS / 8);
// scale after interpolation
//      arm_scale_f32(iFFT_buffer, 2.0, iFFT_buffer, BUFFER_SIZE * N_BLOCKS / 4);
//      arm_scale_f32(&iFFT_buffer[1024], 2.0, &iFFT_buffer[1024], BUFFER_SIZE * N_BLOCKS / 4);
// interpolation-by-4
//      arm_fir_interpolate_f32(&FIR_int2_I, iFFT_buffer, float_buffer_L_original, BUFFER_SIZE * N_BLOCKS / 4);
//      arm_fir_interpolate_f32(&FIR_int2_Q, &iFFT_buffer[1024], float_buffer_R_original, BUFFER_SIZE * N_BLOCKS / 4);
      arm_fir_interpolate_f32(&FIR_int2_I, float_buffer_L_2, float_buffer_L_original, BUFFER_SIZE * N_BLOCKS / 4);
      arm_fir_interpolate_f32(&FIR_int2_Q, float_buffer_R_2, float_buffer_R_original, BUFFER_SIZE * N_BLOCKS / 4);
// scale after interpolation
      arm_scale_f32(float_buffer_L_original, 8.0, float_buffer_L_original, BUFFER_SIZE * N_BLOCKS);
      arm_scale_f32(float_buffer_R_original, 8.0, float_buffer_R_original, BUFFER_SIZE * N_BLOCKS);
 ...
 **********************************************************************************/
    for (i = 0; i < N_BLOCKS; i++)
    {
      ...
      arm_float_to_q15 (&float_buffer_L_original[BUFFER_SIZE * i], sp_L, BUFFER_SIZE); 
      arm_float_to_q15 (&float_buffer_R_original[BUFFER_SIZE * i], sp_R, BUFFER_SIZE); 
      ...
      */
      ...
      { // interleave real and imaginary input values [real, imag, real, imag . . .]
        ...
        // cosf is much much faster than arm_cos_f32 !
          buffer_spec_FFT[i * 2] = 0.5 * (float32_t)((1 - (cosf(PI*2 * (float32_t)i / (float32_t)(512-1)))) * float_buffer_L_original[i]);
          buffer_spec_FFT[i * 2 + 1] = 0.5 * (float32_t)((1 - (cosf(PI*2 * (float32_t)i / (float32_t)(512-1)))) * float_buffer_R_original[i]);        
        // Nuttall window
         buffer_spec_FFT[i * 2] = (0.355768 - (0.487396*cosf((2*PI*(float32_t)i)/(float32_t)512 - 1)) +
                   (0.144232*cosf((4*PI*(float32_t)i)/(float32_t)512 - 1)) - (0.012604*cosf((6*PI*(float32_t)i)/(float32_t)512 - 1))) * float_buffer_L_original[i];
         buffer_spec_FFT[i * 2 + 1] = (0.355768 - (0.487396*cosf((2*PI*(float32_t)i)/(float32_t)512 - 1)) +
                   (0.144232*cosf((4*PI*(float32_t)i)/(float32_t)512 - 1)) - (0.012604*cosf((6*PI*(float32_t)i)/(float32_t)512 - 1))) * float_buffer_R_original[i];
...
/*    // this takes 38.55% and 1kbyte RAM 
     float32_t mags[256];
...
*/            
      ...
      for(int16_t x = 0; x < 256; x++)
      {
           ...
           FFT_spec_old[x] = help; 
              ...
              // but if you use log10f, it´s close to nothing ;-) lol
...
void buttons() {
      ...
      button6.update(); // filter encoder button
            ...
            if ( button4.fallingEdge()) { // cycle thru sample rates
                ...
                show_bandwidth(demod_mode, LP_F_help);
                set_SAM_PLL(zeta, omegaN);
/****************************************************************************************
 *  Recalculate decimation and interpolation FIR filters
 ****************************************************************************************/
    // Decimation filter 1, M1 = 4
    calc_FIR_coeffs (FIR_dec1_coeffs, 25, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate);
    // Decimation filter 2, M2 = 2
    calc_FIR_coeffs (FIR_dec2_coeffs, 44, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4);
    // Interpolation filter 1, L1 = 2
    calc_FIR_coeffs (FIR_int1_coeffs, 8, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4);
    // Interpolation filter 2, L2 = 4
    calc_FIR_coeffs (FIR_int2_coeffs, 4, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate);
            }
...
void set_SAM_PLL(float32_t zeta, float32_t omegaN) {
// DX adjustments: zeta = 0.15, omegaN = 100.0
// very stable, but does not lock very fast
// standard settings: zeta = 1.0, omegaN = 250.0
// maybe user can choose between slow (DX), medium, fast SAM PLL
...
omega_min = TPI * pll_fmin * DF / SR[SAMPLE_RATE].rate;
omega_max = TPI * pll_fmax * DF / SR[SAMPLE_RATE].rate;
g1 = 1.0 - exp(-2.0 * omegaN * zeta * DF / SR[SAMPLE_RATE].rate);
g2 = - g1 + 2.0 * (1 - exp(- omegaN * zeta * DF / SR[SAMPLE_RATE].rate) * cosf(omegaN * DF / SR[SAMPLE_RATE].rate * sqrtf(1.0 - zeta * zeta)));

mtauR = exp(- DF / (SR[SAMPLE_RATE].rate * tauR)); //0.99948;
onem_mtauR = 1.0 - mtauR;
mtauI = exp(- DF / (SR[SAMPLE_RATE].rate * tauI)); //0.99999255955;
