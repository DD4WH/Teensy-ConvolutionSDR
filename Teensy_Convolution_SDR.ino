 ...
 * - automatic test whether mirror rejection is working - if not, codec is restarted automatically until we have working mirror rejection
 *  
 * TODO:
 ...
 * Thanks to bmillier for helping with codec restart code for the SGTL 5000 codec in the Teensy audio board!
 * 
...
#define MENU_VOLUME                       16
#define MENU_TREBLE                       17
#define MENU_BASS                         18
#define MENU_SAM_ZETA                     19
#define MENU_SAM_OMEGA                    20
#define MENU_SAM_CATCH_BW                 21
#define MENU_AGC_MODE                     22
#define MENU_AGC_THRESH                   23
#define MENU_AGC_DECAY                    24
#define MENU_AGC_SLOPE                    25
#define MENU_AGC_HANG_ENABLE              26
#define MENU_AGC_HANG_TIME                27
#define MENU_AGC_HANG_THRESH              28
#define first_menu2                       15
#define last_menu2                        25  
...
float32_t deemp_alpha = 0.1; // 60µsec in Europe, 75µsec in the US 
...
// get audio samples from the audio  buffers and convert them to float
    ...
    sp_R = Q_in_R.readBuffer();
     ...
     { 
         if(i == 0)
         {
            FFT_buffer[i] = 0.025 * atan2f(I_old * float_buffer_R[i] - float_buffer_L[i] * Q_old, 
...
//            rawFM[i] = 0.025 * atan2_fast(float_buffer_L[i - 1] * float_buffer_R[i] - float_buffer_L[i] * float_buffer_R[i - 1], 
     ...
     for(i = 0; i < BUFFER_SIZE * WFM_BLOCKS;i++)
     { 
      FFT_buffer[i] = onem_deemp_alpha * rawFM_old + deemp_alpha * FFT_buffer[i];
     }
...
********************************************************************************************************/
    ...
    {   // Moseley, N.A. & C.H. Slump (2006): A low-complexity feed-forward I/Q imbalance compensation algorithm.
        ...
        if(twinpeaks_counter > 100) // wait 0.667ms for the system to settle: with 32 IQ samples per block and 48ksps (0.66667ms/block)
        {
            twinpeaks_tested = 0;
            twinpeaks_counter = 0;
...
//            tft.drawFastHLine(pos_x_smeter+s*s_w+1, pos_y_smeter+5, nine_sw_minus, ILI9341_BLACK);
            ...
            if(agc_action == 1)
            {
                tft.setCursor(pos_x_dbm + 100, pos_y_dbm);
                tft.setFont(Arial_11);
                tft.setTextColor(ILI9341_WHITE);
                tft.print("AGC");
            }
            ...
            {
                tft.fillRect(pos_x_dbm + 100, pos_y_dbm, 100, 20, ILI9341_BLACK);
            }
