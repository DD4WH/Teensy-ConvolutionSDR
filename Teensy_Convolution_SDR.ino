...
#define Si_5351_crystal 25000000  // 27000000
...
#define       DEMOD_DSB           6 // double sideband: SSB without eliminating the opposite sideband
#define       DEMOD_STEREO_DSB    7 // double sideband: SSB without eliminating the opposite sideband
#define       DEMOD_SAM           8 // synchronous AM demodulation
#define       DEMOD_AM_USB        9 // AM demodulation with lower sideband suppressed
#define       DEMOD_AM_LSB       10 // AM demodulation with upper sideband suppressed
#define       DEMOD_AM_LYONS     11 // AM demodulation with simplified demodulator
...
//  77500 ,"VLF", DEMOD_AM, 3600,3600,0,  
  225000,"LW", DEMOD_AM, 3600,3600,0,
  531000,"MW",  DEMOD_AM, 3600,3600,0,
  2485000,"120M",  DEMOD_AM, 3600,3600,0,
  3500000,"90M",  DEMOD_LSB, 3600,3600,6,
  3905000,"75M",  DEMOD_AM, 3600,3600,4,
  5025000,"60M",  DEMOD_AM, 3600,3600,7,
  5981000,"49M",  DEMOD_AM, 3600,3600,0,
  7120000,"41M",  DEMOD_AM, 3600,3600,0,
  9420000,"31M",  DEMOD_AM, 3600,3600,0,
  11735000,"25M", DEMOD_AM, 3600,3600,0,
  13570000,"22M", DEMOD_AM, 3600,3600,0,
  15140000,"19M",  DEMOD_AM, 3600,3600,0,
  17480000,"16M", DEMOD_AM, 3600,3600,0,
  18900000,"15M", DEMOD_AM, 3600,3600,0,
  21450000,"13M", DEMOD_AM, 3600,3600,0,
  25670000,"11M", DEMOD_AM, 3600,3600,0
};
...
const DEMOD_Descriptor DEMOD [9] =
{
    //   DEMOD_n, name
    {  DEMOD_USB, " USB  "}, 
    {  DEMOD_LSB, " LSB  "}, 
...
void setup() {
  ...
  sgtl5000_1.eqBands (0.5, -0.1); // (float bass, float treble) in % -100 to +100
 ...
 *  this saves about 3% of processor power compared to calculating the magintudes and means of 
...
// which one and how they are combined is dependent upon the demod_mode . . .
      ...
      for(i = 0; i < BUFFER_SIZE * N_BLOCKS; i++)
      {
        if(demod_mode == DEMOD_USB || demod_mode == DEMOD_LSB || demod_mode == DEMOD_DCF77)
        {
...
void encoders () {
  ...
  if (encoder_pos != last_encoder_pos){
    ...
    long tune_help1 = tunestep * round((float32_t)encoder_change / 4.0);
    bands[band].freq += (long)tune_help1;  // tune the master vfo 
