...
uint8_t SAMPLE_RATE =            SAMPLE_RATE_88K; 
...
float32_t IQ_amplitude_correction_factor = 1.003;
float32_t IQ_phase_correction_factor = - 0.0102;
...
#define MENU_IQ_AMPLITUDE                 2
#define MENU_IQ_PHASE                     3  
#define MENU_LPF_SPECTRUM                 4
#define MENU_SAVE_EEPROM                  5
#define MENU_LOAD_EEPROM                  6
#define MENU_CALIBRATION_FACTOR           7
#define MENU_CALIBRATION_CONSTANT         8

#define first_menu                        0
#define last_menu                         1
#define start_menu                        1
uint8_t Menu =                            start_menu;
String menu_text = "Filter Bandwidth";
...
#define RB_SIZE       (int) (MAX_SAMPLE_RATE * MAX_N_TAU * MAX_TAU_ATTACK + 1)
            ...
            uint8_t AGC_mode = 4;
            int pmode = 0; // if 0, calculate magnitude by max(|I|, |Q|), if 1, calculate sqrtf(I*I+Q*Q)
//            double* in,
//            double* out,
//            int io_buffsize,
//            int sample_rate,
...
/**********************************************************************************
 *  AGC - automatic gain control [and S-Meter data collection]
 *  
...
// which one and how they are combined is dependent upon the demod_mode . . .
      ...
      {   // taken from Warren Pratt´s WDSP, 2016
        ...
        for(i = 0; i < FFT_length / 2; i++)
        {
            ...
            if(demod_mode == DEMOD_SAM_STEREO)
            {            
              if(fade_leveler)
              {
...
// scale after interpolation
      ...
      arm_scale_f32(float_buffer_R, 8.0, float_buffer_R, BUFFER_SIZE * N_BLOCKS);
 ...
 * // AGC
 rxa[channel].agc.p = create_wcpagc (
    ...
    n_tau = 4;                        // n_tau
    max_gain = 1; // 10000.0,                    // max_gain
    var_gain = 1.5 / 10000.0; //,                      // var_gain
    fixed_gain = 0.5; //1000.0,                     // fixed_gain
    max_input = 1.0;                 // max_input
    out_target = 1.0;                // out_target
 ...
 */
    ...
    var_gain = 0.0015;  // this is 10^(slope / 20) --> for 10dB slope, this is 3.0 --> doesnt work !?
    fixed_gain = 1.0;                     // fixed_gain
    max_input = 1.0;                 // max_input
    out_targ = 0.0005; //0.0001; // 1.0                // out_target
//    out_targ = 1.0 / 32768.0; //0.0001; // 1.0                // out_target
//    out_targ = 1.0;                // out_target
    tau_fast_backaverage = 0.250;    // tau_fast_backaverage
    tau_fast_decay = 0.005;          // tau_fast_decay
    pop_ratio = 5.0;                 // pop_ratio
...
//  out_target = out_target * (1.0 - expf(-(float32_t)n_tau)) * 0.9999;
  ...
  Serial.println(out_target * 1000000);
...
//      gain = volts * inv_out_target;
      mult = (out_target - slope_constant * min (0.0, log10(inv_max_input * volts))) / volts;
//      Serial.println(mult * 1000000000000);
...
void set_gain()
{
    ...
    static float32_t volume = 0.4;
    static float32_t digital_gain = 1.0;
//    pot = pot / 1023.0 * 1000;
    pot = pot / 1023.0;
    uint16_t pot_int = (uint16_t) pot;
    analog_gain = (uint8_t)(pot * 15);
    sgtl5000_1.lineInLevel(analog_gain); 
...
*/
/*    // other way round: first digital gain, then analog gain
    if(pot < 0.5)
    {
            if (analog_gain != 1)
            {
    ...
    else
    {
            ...
            analog_gain = (uint8_t)((pot - 0.5) * 30);
            sgtl5000_1.lineInLevel(analog_gain); 

    }
*/
/*
// Third experiment: use digital gain before the DAC to avoid pops    
    if(pot_int <= 500)
    {
            if (analog_gain != 0)
            {
...
//           if ((uint8_t)(pot / 1000) % 20 != (uint8_t)(digital_gain / 100) % 20)
            {
            digital_gain = 2.0 * (float32_t)(pot_int / 10 % 100) / 100;
            sgtl5000_1.dacVolume(digital_gain);
            }
    }
    else
    {
            ...
            analog_gain = (uint8_t)(((float32_t)(pot_int - 500) / 1000.0) * 30.0);
            sgtl5000_1.lineInLevel(analog_gain); 
      
    }
*/
//            Serial.print("ANALOG gain: "); Serial.println(analog_gain);
//            Serial.print("DIGITAL gain: "); Serial.println(digital_gain);
//            Serial.print("Volume: "); Serial.println(volume);
//              Serial.print("Pot_int: "); Serial.println(pot_int);
...
void Zoom_FFT_exe (uint32_t blockSize)
{
   ...
   * here, ZOOM FFT works like this: this routine is called at different places of the audio chain:
   * therefore the audio arrives here in different stages of decimation
   * ARRIVE stage is mentioned first []:
   * Magnification 1: [no decimation], take first 256 samples out of 4096
   * Mag 2: [no decimation] decimate-by-2, take first 256 samples
   * Mag 4: [decimated-by-4], take first 256 samples
   * Mag 8: [decimated-by-8], take first 256 samples
   * Mag 16: [decimated-by-8], decimate-by-2, take 256 samples (all of them ;-)
   * Mag 32: [decimated-by-8], decimate-by-4, do this for two FFT [1024] loops, then take 256 samples
   * Mag 64: [decimated-by-8], decimate-by-4, do this for four FFT [1024] loops, then take 256 samples
   *********************************************************************************************/
  ...
  // and displays the bandwidth bar indicating demodulation bandwidth
   ...
   tft.drawFastHLine(spectrum_x, pos_y + 3, 256, ILI9341_BLACK); // erase old indicator
    ...
    switch(M) {
      ...
      if((leU + spectrum_pos_centre_f) > 255) leU = 255 - spectrum_pos_centre_f;
 ...
 ****************************************************************************************/
    // Decimation filter 1, M1 = 4
    calc_FIR_coeffs (FIR_dec1_coeffs, 50, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate);
    // Decimation filter 2, M2 = 2
    calc_FIR_coeffs (FIR_dec2_coeffs, 88, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4);
    // Interpolation filter 1, L1 = 2
    calc_FIR_coeffs (FIR_int1_coeffs, 16, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate / 4);
    // Interpolation filter 2, L2 = 4
    calc_FIR_coeffs (FIR_int2_coeffs, 16, (float32_t)SR[SAMPLE_RATE].rate / 17.0, 80, 0, 0.0, SR[SAMPLE_RATE].rate);
            AudioInterrupts();
            }
            ...
            if (button7.fallingEdge()) { 
               ...
               Serial.println("MENU BUTTOON pressed");
               // show_menu();
            }            
}
...
////////////////////////////////////////////////
  if (encoder2_pos != last_encoder2_pos){
    ...
    if (Menu == MENU_IQ_AMPLITUDE) 
    {
          IQ_amplitude_correction_factor += encoder2_change / 10000.0;
    ...
    if (Menu == MENU_LPF_SPECTRUM) 
    {
    } // END LPFSPECTRUM
    else 
    if (Menu == MENU_IQ_PHASE) 
    {
          IQ_phase_correction_factor = IQ_phase_correction_factor + (float32_t)encoder2_change / 10000.0;
...
void FFT_Zoom_prepare_IIR(uint8_t mag) {
  
}
/*
void FFT_Zoom_Process_Samples(const uint16_t blockSize)
{
    if(FFT_Zoom_state == 0)
    {
        if(FFT_Zoom_magnification != 0)        //
            // magnify 2, 4, 8, 16, or 32
        {
            // ZOOM FFT
            // is used here to have a very close look at a small part
            // of the spectrum display
            // The ZOOM FFT is based on the principles described in Lyons (2011)
            // 1. take the I & Q samples
            // 2. complex conversion to baseband (at this place has already been done)
            // 3. lowpass I and lowpass Q separately (sample rate-dependence)
            // 4. decimate I and Q separately
            // 5. apply 256-point-FFT to decimated I&Q samples
            //
            // frequency resolution: spectrum bandwidth / 256
            // example: decimate by 8 --> 96kHz / 16 = 6kHz spectrum display bandwidth
            // frequency resolution of the display --> 6kHz / 256 = 23.44Hz
            // in 32x Mag-mode the resolution is only a few Hz, not bad for such a small processor . . .
            //
            ...
            // biquad lowpass filters with 4 stages each
            ...
            for(int i = 0; i < blockSize/ (1<<FFT_Zoom_magnification); i++)
            {
